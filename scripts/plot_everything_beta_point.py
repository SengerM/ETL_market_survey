...
def script_core(directory: Path):
	...
	with John.do_your_magic():
		...
		for column in measured_data_df:
			...
			fig.write_html(
				str(John.path_to_default_output_directory/Path(f'{column} histogram.html')),
			...
			fig.write_html(
				str(John.path_to_default_output_directory/Path(f'{column} ECDF.html')),
				include_plotlyjs = 'cdn',
			)
		...
		fig.write_html(
			str(John.path_to_default_output_directory/Path('scatter matrix')) + '.html',
			include_plotlyjs = 'cdn',
		)
